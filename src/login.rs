...
// ── OAuth redirect capture (loopback listener) ────────────────────────
...
/// Open the browser and wait for the OAuth redirect back to the configured
...
pub async fn login_twitch(eng: &EngineHandle, channel: &str, cfg: &ChatConfig) -> Result<LoginState, String> {
...
pub async fn login_and_verify(
    ...
    let mut state = match platform {
        "twitch" => login_twitch(eng, channel, cfg).await?,
        "kick" => login_kick(eng, kick_handle, cfg).await?,
